    ...
    <div className="flex-1 w-full md:border-r border-[#2a2a2a] last:border-0 md:pr-4 last:pr-0 mb-8 md:mb-0">
      ...
      <div className="flex flex-col gap-3">
        {games.map((game) => {
          ...
          return (
            <Link key={game.title} to={`/game/${folderName}?from=${endpoint}`}
              className="flex items-center gap-4 p-2 rounded-lg hover:bg-[#2a2a2a] group">
              <div className="relative w-[60px] h-[80px] flex-none rounded-md overflow-hidden bg-[#111]">
                <img src={imageSrc} alt={game.title} className="object-cover" />
                ...
                <div className="absolute top-1 right-1 md:opacity-0 md:group-hover:opacity-100">
                  <button onClick={(e) => {
                    ...
                    <Bookmark size={12} className={inWishlist ? "fill-white" : ""} />
                  </button>
                </div>
              </div>
              <div className="flex flex-col justify-center min-w-0">
                ...
                <div className="flex items-center gap-2 flex-wrap">
                  {game.discount && (
                    <span className="bg-[#26bbff] text-black text-[11px] font-bold px-2 py-1 rounded">{game.discount}</span>
                  )}
                  {game.oldPrice && (
