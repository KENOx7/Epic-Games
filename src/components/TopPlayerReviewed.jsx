    ...
    <div className="max-w-[1200px] mx-auto mt-10 px-4">
      <div className="flex items-center justify-between mb-6">
        <Link to="/browse?category=top-player-reviewed" className="flex items-center group">
          <h2 className="text-white text-xl font-bold">{t("Top Player Reviewed")}</h2>
      ...
      <div className="overflow-hidden" onTouchStart={handleTouchStart} onTouchEnd={handleTouchEnd}>
        <div className="flex gap-5 transition-transform duration-500 ease-out" style={{ transform: `translateX(-${scrollIndex * 198}px)` }}>
          {games.map((game) => {
            ...
            const img = `https://epic-games-api-eta.vercel.app/top-player-reviewed/${slug}/cover.jpg`
            const active = isInWishlist(game.title)
            return (
              <Link key={game.title} to={`/game/${slug}?from=top-player-reviewed`} className="block w-[178px] flex-none group">
...
export default TopPlayerReviewed