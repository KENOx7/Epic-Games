...
function Header() {
  ...
  useEffect(() => {
    const loadGames = async () => {
      ...
      allGames.forEach((game) => {
        if (!uniqueGames.find((item) => item.title === game.title)) {
  ...
  const results = searchValue
    ? games.filter((game) => {
      ...
      }).slice(0, 5) // Çox nəticə çıxsa da, axtarış pəncərəsində yalnız ilk 5 oyunu göstəririk
    : []
    ...
    <>
      ...
      <header className="relative z-[60] bg-[#121216] h-[72px] w-full flex items-center justify-between p-5">
        ...
        <div className="flex items-center gap-5 shrink-0">
          <Link to="/" className="flex items-center gap-5 hover:opacity-80 shrink-0">
            <img src={logo} alt="logo" className="h-[40px] w-auto shrink-0" />
            <img src={store} alt="store" className="w-[54px] h-[32px] shrink-0" />
          </Link>
              ...
              <div className="absolute right-0 top-10 bg-[#18181c] rounded-md shadow-xl py-2 w-[150px] z-50">
                {languages.map((item) => (
                  <button key={item.code} onClick={() => changeLanguage(item.code)}
                    className={`w-full text-left px-5 py-2.5 hover:bg-[#2a2a30] ${language === item.code ? "text-white font-bold" : "text-[#AEAEAF]"}`}>
                    {item.label}
                  </button>
                ))}
              </div>
      ...
      {menuOpen && (
        <div className="md:hidden bg-[#121216] fixed inset-0 z-[60] flex flex-col">
          ...
          <div className="h-[72px] w-full flex items-center justify-between p-5">
            ...
            <button onClick={() => setMenuOpen(false)} className="text-white hover:opacity-60"><X size={24} /></button>
          </div>
          ...
          <div className="flex flex-col px-5 pb-5 gap-6">
            <div className="flex justify-end items-center mt-2">
              <div className="flex items-center gap-4">
                <div className="flex gap-4">
                  {languages.map((item) => (
                    <button key={item.code} onClick={() => setLanguage(item.code)} className={`text-sm ${language === item.code ? "text-white font-bold" : "text-gray-400"}`}>
                      {item.short}
                    </button>
                  ))}
                </div>
            ...
            <ul>
              ...
              <li className="pt-10"><Link to="/support" onClick={() => setMenuOpen(false)} className="text-white hover:text-gray-300">{t("support")}</Link></li>
              <li className="pt-5"><Link to="/" onClick={() => setMenuOpen(false)} className="text-white hover:text-gray-300">{t("distribute")}</Link></li>
            </ul>
          </div>
        </div>
      )}
        ...
        <div className="max-w-[1200px] mx-auto h-full flex justify-between items-center px-4 xl:px-0">
                  ...
                  {results.length > 0 ? (
                    <>
                      ...
                      {results.map((game) => {
                        ...
                        return (
                          <Link key={game.title} to={`/game/${folderName}?from=${game.endpoint}`} onClick={closeSearch} className="flex items-center gap-3 px-4 py-2.5 hover:bg-[#2a2a30]">
          ...
          <div className="flex items-center gap-6 text-[#AEAEAF]">
            ...
            <Link to="/wishlist" className="relative hover:text-white flex items-center">
              ...
              {wishlist.length > 0 && (
                <span className="absolute -top-2 -right-2 bg-white text-black text-[10px] font-bold w-4 h-4 flex items-center justify-center rounded-full">{wishlist.length}</span>
              )}
            </Link>
            ...
            <Link to="/cart" className="relative hover:text-white flex items-center">
              ...
              {cart.length > 0 && (
                <span className="absolute -top-2 -right-2 bg-white text-black text-[10px] font-bold w-4 h-4 flex items-center justify-center rounded-full">{cart.length}</span>
              )}
            </Link>
          </div>
        </div>
      ...
      {mobileSearchOpen && (
        <div className="fixed inset-0 bg-[#121216] z-[60] flex flex-col p-5">
          <div className="flex justify-between items-center mb-6">
            ...
            <button onClick={closeSearch} className="text-[#AEAEAF] hover:text-white"><X size={28} /></button>
          </div>
